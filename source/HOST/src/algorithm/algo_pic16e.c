...
void print_pic16e_error(int errc)
{
	printf("\n");
	switch(errc)
	{
...
//		printf("bsize= %04X  addr= %04X\n",bsize,addr);
	...
	if((conf_prog == 1) && (errc == 0))
	{
		...
		printf("CONF= %02X%02X %02X%02X %02X%02X %02X%02X %02X%02X\n",memory[1],memory[0],
		memory[3],memory[2],memory[5],memory[4],memory[7],
		memory[6],memory[9],memory[8]);
...
//		printf("bsize= %04X  addr= %04X\n",bsize,addr);
		...
		memory[3+ROFFSET],memory[2+ROFFSET],memory[5+ROFFSET],memory[4+ROFFSET],memory[7+ROFFSET],
		memory[6+ROFFSET],memory[9+ROFFSET],memory[8+ROFFSET]);
	}
