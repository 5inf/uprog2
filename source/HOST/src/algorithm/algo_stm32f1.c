...
void print_stm32f1_error(int errc)
{
	printf("\n");
	switch(errc)
	{
...
//				prg_comm(0x0f,0,0,0,0,0,0,0,0);			//exit
		...
		if((run_ram == 0) && (errc == 0) && ((main_prog == 1) || (option_prog == 1) || (unsecure == 1)))
		{
			printf("TRANSFER LOADER...");
			for(j=0;j<512;j++)
			{
				memory[j]=exec_stm32f1xx[j];
			}
