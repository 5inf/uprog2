...
#include <main.h>
#include "exec/samd21/exec_samd21.h"

void print_samdswd_error(int errc)
{
	printf("\n");
	switch(errc)
	{
...
void show_samdswd_registers(void)
{
	int i;
	for(i=0;i<13;i++)
	{
...
samdswd_ORUN:
	...
	if(dev_start == 0)
	{
		...
		{
			errc=prg_comm(0x1D0,0,16,0,0,0,0,0xDE,0x06);	//init, reset HIGH, no debug core start
			printf("JID: %02X%02X%02X%02X\n",memory[3],memory[2],memory[1],memory[0]);
			if(errc > 0) goto ERR_EXIT;
			printf("ERASE FLASH\n");

			errc=prg_comm(0x1D1,0,5,0,0,0x18,0x20,0x00,0x41);				//READ DEVID
			printf("ACK= %02X\n",memory[4]);
			printf("DID: %02X%02X%02X%02X\n",memory[3],memory[2],memory[1],memory[0]);


			errc=prg_comm(0x1D1,0,5,0,0,0x00,0x00,0x00,0x41);				//READ DEVID
			printf("ACK= %02X\n",memory[4]);
			printf("WPCLR: %02X%02X%02X%02X\n",memory[3],memory[2],memory[1],memory[0]);
			...
			memory[3]=0x41;memory[2]=0x00;memory[1]=0x00;memory[0]=0x00;	//addr
			memory[7]=0x00;memory[6]=0x00;memory[5]=0x00;memory[4]=0x02;	//data
			errc=prg_comm(0x23D,8,1,0,0,0,0,0,0);				//write long
			printf("ACK= %02X\n",memory[0]);
...
*/
			...
			memory[3]=0x41;memory[2]=0x00;memory[1]=0x20;memory[0]=0x00;	//addr (CTRL)
			memory[7]=0x00;memory[6]=0x00;memory[5]=0x00;memory[4]=0x10;	//data
//			errc=prg_comm(0x23F,8,1,0,0,0,0,0,0);				//write byte
			printf("ACK= %02X\n",memory[0]);

			waitkey();


//			errc=prg_comm(0x252,0,4,0,0,0,0,0,0);		//erase direct
//			printf("MASS ERASE_TIME: %d ms\n",(20-memory[0])*20);
			if(errc > 0) goto ERR_EXIT;
			printf("RE-INIT\n");
			errc=prg_comm(0x1D0,0,16,0,0,0,0,0xDE,0x03);		//re-init
		}
		else
		{
			errc=prg_comm(0x1D0,0,16,0,0,0,0,0xDE,0x03);					//init
			printf("JID: %02X%02X%02X%02X\n",memory[3],memory[2],memory[1],memory[0]);
//			printf("ERR= %02X\n",errc);
			if(errc > 0) goto ERR_EXIT;
		}
...
*/
		...
		if((run_ram == 0) && (errc == 0) && ((main_prog == 1) || (eeprom_prog == 1) || (eeprom_readout == 1) || (eeprom_verify == 1)))
		{
			...
			for(j=0;j<512;j++)
			{
				switch(algo_nr)
				{
					case 56:	memory[j]=exec_samd21[j]; break;
